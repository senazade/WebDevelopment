...
const CreateDonation = async (req, res, next) => {
  ...
  try {
    const MakeADonation = 'INSERT INTO Donations (institution_name, address, budget, transaction_number) VALUES (?,?,?,?)';
    con.query(MakeADonation, [institution_name, address, budget, transaction_number], (err, results) => {
      if (err) {
        console.error(err);
      } else {
        console.log(results);
    ...
    res.status(201).send({ success: true, message: 'Successfully created a donation', data: {institution_name, address, budget, transaction_number} });
  } catch (error) {
    res.status(500).send({ success: false, message: 'Server error' });
  }
